...
static void print_pcn_rule(const IPT_CHAINLABEL chain, const char *action, const struct ipt_entry *rule, unsigned int rulenum, struct xtc_handle *handle){
	...
	if (prerouting_match == 0 || postrouting_match == 0){
		...
		if (replace_match == 0){
			printf("iptables nat %s add %d", chain, action, rulenum);
		} else if (insert_match == 0){
			printf("iptables nat %s %s id=%d", chain, action, rulenum);
		}else if (flush_match == 0){
			printf("iptables nat %s rule del", chain);
			return;
		}else{
			printf("iptables nat %s %s ", chain, action);
			if (rulenum!=-1){
				printf("%d ", rulenum + 1);
			}
		}
	...
	{
		if (replace_match == 0){
			printf("iptables chain %s add %d", chain, action, rulenum);
		} else if (insert_match == 0){
			printf("iptables chain %s %s id=%d", chain, action, rulenum);
		}else if (flush_match == 0){
			printf("iptables chain %s rule del", chain);
			return;
		}else if ((delete_match == 0) && (rulenum!=-1)){
			printf("iptables chain %s rule del %d", chain, rulenum);
			return;
		}else{
			printf("iptables chain %s %s ", chain, action);
			if (rulenum!=-1){
				printf("%d ", rulenum + 1);
			}
		}
...
static void print_pcn_policy(const IPT_CHAINLABEL chain, const IPT_CHAINLABEL policy, const char *action){
	...
	printf("iptables chain %s set default=%s ", chain, policy);
